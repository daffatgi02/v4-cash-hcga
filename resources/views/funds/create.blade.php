                    ...
                    <form action="{{ route('funds.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                <option value="" selected disabled>Pilih Tipe Dana</option>
                                <option value="rekening_utama" {{ old('type') == 'rekening_utama' ? 'selected' : '' }}>Rekening Utama</option>
                                <option value="kas_kecil" {{ old('type') == 'kas_kecil' ? 'selected' : '' }}>Kas Kecil</option>
                                <option value="dana_operasional" {{ old('type') == 'dana_operasional' ? 'selected' : '' }}>Dana Operasional</option>
                                <option value="dana_rkb" {{ old('type') == 'dana_rkb' ? 'selected' : '' }}>Dana RKB</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control rupiah @error('initial_balance') is-invalid @enderror" id="initial_balance" name="initial_balance" value="{{ old('initial_balance', '0') }}" required>
